...
def render_ohlcv_section(
    ...
    with tabs[1]:
        ...
        # --- KD Card ---
        st.markdown("**KD 指标** &nbsp;&nbsp;&nbsp; <small style='color:#888;font-weight:normal;'>"
                    "K,D 双线 > 80 超买 | K,D 双线 < 20 超卖 | 背离周期边界 = 20 / 80 | "
                    "|K-D| ≥ 20 大概率收敛</small>", unsafe_allow_html=True)
        st.html(f"""
        ...
        _rsi_caption = ("RSI > 70 超买 | RSI < 30 超卖 | 背离周期边界 = 50 | "
                        "多头 RSI>K=强度充足 | 空头 RSI<K=强度不足") if rsi_val is not None else "RSI 数据不足"
        st.markdown(f"**RSI 指标** &nbsp;&nbsp;&nbsp; <small style='color:#888;font-weight:normal;'>{_rsi_caption}</small>",
                    unsafe_allow_html=True)
        ...
        if section_type != "stock":
            st.markdown("**BIAS 乖离率** &nbsp;&nbsp;&nbsp; <small style='color:#888;font-weight:normal;'>"
                        "10日乖离 > 10 短线超买 | < -10 短线超卖 | 月线乖离(20日) > 7 超买 | < -7 超卖</small>",
                        unsafe_allow_html=True)
    ...
    # ── Tab 2: 权重&成分 (index/industry only) ──
    if section_type != "stock":
        with tabs[2]:
            # ── 成分股分析 (industry) ──
