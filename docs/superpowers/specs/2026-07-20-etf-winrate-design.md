...
### 2.2 过滤规则（6 条，已用真实数据验证：8000 → 727）

```python
# 过滤后：行业指数 212 + 主题指数 515 = 727 个可回测指数
# (数量随 index_basic 当日返回动态变化，727 为 2026-07-20 快照)
...
**完整名单**：`csi_index_pool_kept.txt`（727 个保留）+ `csi_index_pool_removed.txt`（2636 个被过滤）已导出到项目根目录，供用户 review。

**已知小尾巴（不处理）**：727 个里零星几个噪音（`三板医药`/`三板消费`新三板流动性差、`央企100`/`地企100`/`国企200`按企业属性切的、`证券时报ESG百强`等定制指数）。跑回测时 `n < 60` 自然淘汰，不值得为它们加规则。
