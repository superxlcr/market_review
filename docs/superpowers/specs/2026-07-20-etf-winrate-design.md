...
### 2.2 过滤规则（6 条，已用真实数据验证：8000 → 709）

```python
# 过滤后：行业指数 204 + 主题指数 505 = 709 个可回测指数
# (数量随 index_basic 当日返回动态变化，709 为 2026-07-20 快照)
...
**完整名单**：`csi_index_pool_kept.txt`（709 个保留）+ `csi_index_pool_removed.txt`（2654 个被过滤）已导出到项目根目录，供用户 review。

**已知小尾巴（不处理）**：709 个里零星几个噪音（`三板医药`/`三板消费`新三板流动性差、`中证新兴`/`中证龙头`等非纯行业、`证券时报ESG百强`等定制指数）。跑回测时 `n < 60` 自然淘汰，不值得为它们加规则。
