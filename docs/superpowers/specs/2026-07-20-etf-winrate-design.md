...
### 2.2 过滤规则（6 条，已用真实数据验证：8000 → 790）

```python
# 过滤后：行业指数 225 + 主题指数 565 = 790 个可回测指数
...
# 3. name 不含全收益/净收益/(全)/(净)
df = df[~df['name'].str.contains(r'全收益|净收益|（全）|(?:全)|(?:净)|（净）', na=False, regex=True)]

# 4. name 不含港股/海外/跨市（A 股账户买不了的）
df = df[~df['name'].str.contains(r'港股|香港|HK|SHS|海外|沪港|深港|沪通|深通|港通|AH', na=False, regex=True)]

# 5. name 不以规模前缀开头（砍 300能源/500能源/800能源 这类同行业规模口径变体；
#    已验证：常见行业银行/医药/电池/新能源/半导体/电力/白酒/军工/煤炭/钢铁/房地产/电子/计算机/传媒
#    全部还有无前缀版存活，无误杀）
df = df[~df['name'].str.match(r'^(?:1000|500|300|180|380|800|700|200|50|100|中证|上证|深证|国证)', na=False)]

# 6. name 不含策略型残留（红利/低波/动量/价值/成长结尾/收结尾）
df = df[~df['name'].str.contains(r'收$|红利|分红|低波|动量|高贝|价值|成长$', na=False, regex=True)]
```

**验证结果**：用户的 4 个示例指数全部保留——
- `931719.CSI` CS电池（主题）✅
- `931152.CSI` CS创新药（主题）✅
- `930851.CSI` 云计算（主题）✅
- `H30199.CSI` 电力指数（行业）✅

**已知小尾巴（不处理）**：790 个里零星几个噪音（`三板医药`/`三板消费`新三板流动性差、`证券时报ESG百强`等定制指数）。跑回测时 `n < 60` 自然淘汰，不值得为它们加规则。
...
ETF 版可测买点**独立定义**为 `ETF_BUY_POINTS`（见 §5.2），不依赖个股版 `ALL_BUY_POINTS`——因为 MA 家族在个股版被 `disabled`，`ALL_BUY_POINTS` 会过滤掉它们，而 ETF 版要测 MA。`BUY_POINT_STAGE`（stage 真相源）仍共用、不改动，个股版不受影响。ETF 页面绕过 stage 门槛，直接把买点名传给 `detect_buy_points`。
...
**设计选择**（待 review 定，见 §11.5）：

- **MA 家族有 9 个变体**，个股版全测过都不行。ETF 版不该盲目全启用 9 个（样本分散、对比难看）。
- **建议**：ETF 版第一轮只启用 **3 个代表性周期**（MA60 / MA120 / MA240 —— 短/中/长各一，均用 `vol_mode="today"`），跑完看哪个周期有效再收窄或扩展。
- **实现方式**：不污染个股版的 `BUY_POINT_STAGE`。ETF 页面的买点可选列表**独立定义**（ETF 版 `ETF_BUY_POINTS = ["回调一半","波段50%","量价节点","MA60支撑","MA120支撑","MA240支撑"]`），不依赖 `ALL_BUY_POINTS`。`detect_buy_points` 本来就按名字查 `_NAME_MAP`，这些 MA 名字已在表中、checker 可用，只是 stage 门槛挡住了——ETF 页面绕过 stage 门槛直接传名字即可。
...
[配置区]
  买点(多选): 回调一半 / 波段50% / 量价节点 / MA60支撑 / MA120支撑 / MA240支撑
  均线排列过滤 / 时间止损天数 / 并发数
...
| 项 | 个股版 06 | ETF版 07 |
|----|-----------|----------|
| 买点 | ALL_BUY_POINTS（含MA家族等） | 四个（回调一半/波段50%/均线支撑/量价节点） |
| 市值过滤 | 有 | **无** |
| 行业白名单 | 有 | **无**（换成指数池多选） |
| 标的池 | 全市场非ST | 选中指数 |
| 调试标的 | 个股下拉 | 指数下拉 |
...
1. **`entry_mode="close"` 第一版只留接口不实现**——确认接受？
2. **量价节点第一版完全复用、不改逻辑**——确认接受？
3. **过滤规则的 6 条**——确认没有误砍你想测的指数类型？（可再抽样核对）
4. **新页面编号 `07`**——确认页面顺序？
5. **MA 买点第一轮启用哪几个周期**——建议 MA60/MA120/MA240 三个（短/中/长，vol_mode=today），还是你想测全部 9 个变体？或只测某一个？见 §5.2。
